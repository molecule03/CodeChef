...
public class C {
    ...
    public static void main(String[] args) {
        ...
        for (int tt=0; tt<T; tt++) {
            ...
            String res = getString(p,q,s);
            out.println(res);
        }
        out.close();
    }

    private static String getString(int p, int q, String s){
        StringBuilder sb = new StringBuilder();
        for(char curChar : s.toCharArray()) {
//                char curChar = s.charAt(i);
            int prefix[] = new int[p];
            while(q != 0 && p == 0) {
                boolean left = (curChar-'a' > 0);
                boolean right = (curChar+p) < 123;
                if(left && right) {
                    curChar--;
                    q--;
                }
                else if((curChar+p) > 122) {
                    p--;
                    curChar++;
                }
                else {
