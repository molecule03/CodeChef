package CodeChef_Contests.Starter84;
...
//        Scanner scanner = new Scanner(System.in);
        ...
        for (int i = 0; i < n; i++) {
            ...
            stringArray2[i] = s;
            for (int j = 0; j < s.length(); j++) {
                digitCount2[s.charAt(j) - '0']++;
            }
        }

        for (int i = 0; i < n; i++) {
            if (stringArray1[i].length() != stringArray2[i].length()) {
                out.println("NO");
                return;
            }
        }

        long sDifference = 0, addCount = 0;
        for (int i = 0; i < 10; i++) {
            if (digitCount1[i] == digitCount2[i]) {
                continue;
            } else if (digitCount1[i] < digitCount2[i]) {
                sDifference += (digitCount2[i] - digitCount1[i]);
            } else {
                addCount += Math.abs(digitCount2[i] - digitCount1[i]);
            }
        }

        if (sDifference != addCount) {
            out.println("NO");
            return;
        }
